...
import {
  FiAlertTriangle, FiSearch, FiFilter, FiChevronLeft, FiChevronRight,
  FiAlertCircle, FiInfo, FiClock, FiEye, FiX, FiServer,
} from 'react-icons/fi';
import { getHistorialSistema, getHistorialModulos, getHistorialEstadisticas } from '../services/historialService';
import Loading from '../components/common/Loading';

const NIVEL_BADGE = {
  error:   { bg: '#fee2e2', color: '#991b1b', icon: FiAlertCircle, label: 'Error' },
  warning: { bg: '#fef3c7', color: '#92400e', icon: FiAlertTriangle, label: 'Warning' },
  info:    { bg: '#dbeafe', color: '#1e40af', icon: FiInfo, label: 'Info' },
};

function HistorialSistema() {
  ...
  const [filtros, setFiltros] = useState({
    buscar: '', modulo: '', nivel: '', fecha_desde: '', fecha_hasta: '', page: 1,
  });
  ...
  const cargar = useCallback(async (params = filtros) => {
    ...
    try {
      ...
      const res = await getHistorialSistema(clean);
      setLogs(res.data || []);
  ...
  const limpiar = () => {
    const f = { buscar: '', modulo: '', nivel: '', fecha_desde: '', fecha_hasta: '', page: 1 };
    setFiltros(f); cargar(f);
  };

  const getBadge = (nivel) => NIVEL_BADGE[nivel] || NIVEL_BADGE.info;

  const formatFecha = (d) => {
    if (!d) return '-';
    const f = new Date(d);
    return f.toLocaleDateString('es-PE', { day: '2-digit', month: '2-digit', year: 'numeric' })
      + ' ' + f.toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  };

  return (
    <div className="page-container">
      <div className="page-header">
        <h2><FiServer style={{ marginRight: 8 }} /> Historial del Sistema</h2>
        <span className="page-subtitle">Errores, advertencias y eventos del sistema</span>
      </div>

      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 20 }}>
        <div className="dash-kpi dash-kpi--red">
          <div className="dash-kpi-icon"><FiAlertCircle size={20} /></div>
          <div><h3>{stats.errores_hoy ?? 0}</h3><p>Errores hoy</p></div>
        </div>
        <div className="dash-kpi dash-kpi--amber">
          <div className="dash-kpi-icon"><FiAlertTriangle size={20} /></div>
          <div><h3>{stats.warnings_hoy ?? 0}</h3><p>Warnings hoy</p></div>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', padding: '12px 16px' }}>
          <div style={{ position: 'relative', flex: '1 1 200px' }}>
            ...
            <input
              ...
              placeholder="Buscar por descripción, módulo..."
          ...
          <select className="form-input" value={filtros.nivel} onChange={(e) => setFiltros(p => ({ ...p, nivel: e.target.value }))}
      ...
      <div className="table-card">
        {loading ? <Loading /> : logs.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: '#94a3b8' }}>
            <FiServer size={40} style={{ marginBottom: 10, opacity: 0.4 }} />
            <p>No se encontraron eventos del sistema</p>
          </div>
        ) : (
          <>
            <div style={{ overflowX: 'auto' }}>
              <table className="data-table">
                ...
                <tbody>
                  {logs.map((log) => {
                    const badge = getBadge(log.nivel);
                    const Icon = badge.icon;
                    return (
                      <tr key={log.id} style={{ background: log.nivel === 'error' ? '#fef2f2' : undefined }}>
                        <td>
                        ...
                        <td style={{ fontSize: '0.82rem', fontWeight: 600 }}>{log.accion}</td>
                        <td style={{ fontSize: '0.82rem', maxWidth: 400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {log.descripcion}
                        </td>
                        <td style={{ fontSize: '0.82rem', color: '#64748b' }}>{log.user_nombre || '-'}</td>
                        <td>
                          <button
      ...
      {detalle && (
        <div className="modal-overlay" onClick={() => setDetalle(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}
            style={{ maxWidth: 650, maxHeight: '80vh', overflow: 'auto' }}>
            <div className="modal-header">
              <h3>Detalle del Evento #{detalle.id}</h3>
              <button className="btn btn-ghost" onClick={() => setDetalle(null)}><FiX size={18} /></button>
            </div>
            <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div><label style={{ fontSize: '0.75rem', color: '#94a3b8', fontWeight: 600 }}>NIVEL</label>
                  <p style={{ fontWeight: 700, color: getBadge(detalle.nivel).color }}>{detalle.nivel?.toUpperCase()}</p></div>
                ...
                <div><label style={{ fontSize: '0.75rem', color: '#94a3b8', fontWeight: 600 }}>USUARIO</label>
                  <p>{detalle.user_nombre || '-'}</p></div>
                <div><label style={{ fontSize: '0.75rem', color: '#94a3b8', fontWeight: 600 }}>IP</label>
                  <p>{detalle.ip || '-'}</p></div>
                <div style={{ gridColumn: '1/-1' }}><label style={{ fontSize: '0.75rem', color: '#94a3b8', fontWeight: 600 }}>URL</label>
                  <p style={{ fontSize: '0.8rem', wordBreak: 'break-all' }}>{detalle.metodo} {detalle.url || '-'}</p></div>
              </div>

              {detalle.datos_antes && (
                <div>
                  <label style={{ fontSize: '0.75rem', color: '#ef4444', fontWeight: 600 }}>DATOS / DETALLE DEL ERROR</label>
                  <pre style={{ background: '#fef2f2', padding: 12, borderRadius: 6, fontSize: '0.75rem', overflow: 'auto', maxHeight: 250, whiteSpace: 'pre-wrap' }}>
              ...
              {detalle.user_agent && (
                <div>
                  ...
                  <p style={{ fontSize: '0.78rem', color: '#64748b', wordBreak: 'break-all' }}>{detalle.user_agent}</p>
                </div>
              )}
            </div>
          </div>
